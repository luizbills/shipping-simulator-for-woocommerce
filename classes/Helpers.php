...
abstract class Helpers {
	...
	public static function get_defined ( $constant, $default = null ) {
	...
	public static function throw_if ( $condition, $message, $exception_class = null ) {
		if ( $condition ) {
			...
			$exception_class = $exception_class ? $exception_class : Plugin_Error::class;
	...
	public static function get_template ( $path, $args = [] ) {
		...
		$templates_dir = \trim( h::config_get( 'TEMPLATES_DIR', 'templates' ), '/' );
		$full_path = h::config_get( 'DIR' ) . "/{$templates_dir}/{$path}";
